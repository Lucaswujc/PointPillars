...
def get_score_thresholds(tp_scores, total_num_valid_gt, num_sample_pts=41):
    score_thresholds = []
    # [ ::-1] is doing reverse order of the list
...
def do_eval(det_results, gt_results, CLASSES, saved_path, skipped_ids):
    ...
    for id in ids:
        ...
        gt_bev = np.concatenate(
            [gt_location[:, [0, 2]], gt_dimensions[:, [0, 2]], gt_rotation_y[:, None]],
            axis=-1,
        )
    ...
    for e_ind, eval_type in enumerate(["bbox_2d", "bbox_bev", "bbox_3d"]):
        ...
        for k, v in eval_ap_results.items():
            ...
            print(
                f"{k} AP@{MIN_IOUS[k][e_ind]}: {v[0]:.4f} {v[1]:.4f} {v[2]:.4f}", file=f
            )
        if eval_type == "bbox_2d":
            print(f"==========AOS==========")
            print(f"==========AOS==========", file=f)
            for k, v in eval_aos_results.items():
                ...
                print(
                    f"{k} AOS@{MIN_IOUS[k][e_ind]}: {v[0]:.4f} {v[1]:.4f} {v[2]:.4f}",
                    file=f,
                )
...
def main(args):
    if args.dequantize_config is not None:
        from pointpillars.utils.quantize import Dequantizer

        dequantizer = Dequantizer(args.dequantize_config)
    else:
        dequantizer = None
    ...
    print(f"pre-check the dataset for errors")
    skipped_ids = []
    skipped_idxes = []
    for idx in tqdm(range(len(val_dataset))):
        try:
            ...
            print(
                f"Error at index {idx}, file {val_dataset.data_infos[val_dataset.sorted_ids[idx]]['velodyne_path']}: {e}"
            )
            # Optionally, log or skip
            skipped_ids.append(val_dataset.sorted_ids[idx])
            skipped_idxes.append(idx)
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            for j, result in enumerate(batch_results):
                ...
                if type(result) != dict:
                    ...
                    bboxes2d, camera_bboxes = (
                        result_filter["bboxes2d"],
                        result_filter["camera_bboxes"],
                    )
                    for lidar_bbox, label, score, bbox2d, camera_bbox in zip(
    ...
    do_eval(
        ...
        gt_results=val_dataset.data_infos,
        CLASSES=CLASSES,
...
def prepare_eval(
    eval_image_path: str,
    kitti_root=os.path.join(current_file_path, "kitti"),
    new_folder=os.path.join(current_file_path, "sparsepcgc_eval"),
):
    """
    create a folder structure that have symbolic link pointing to the eval_image_path
    the folder should mimic the kitti root format
    """
    assert os.path.exists(eval_image_path)
    assert os.path.exists(kitti_root)
    if not os.path.isabs(eval_image_path):
        eval_image_path = os.path.abspath(eval_image_path)
    ...
    train_subfolders = ["image_2", "velodyne_reduced", "label_2", "calib", "velodyne"]
    testing_subfolders = ["image_2", "velodyne_reduced", "calib", "velodyne"]

    train_folder = os.path.join(new_folder, "training")
    testing_folder = os.path.join(new_folder, "testing")
    results_folder = os.path.join(new_folder, "results")
    os.makedirs(results_folder, exist_ok=True)
    os.makedirs(train_folder, exist_ok=True)
    os.makedirs(testing_folder, exist_ok=True)
    # create symbolic links for pkl files
    for pkl in pkl_files:
        os.remove(os.path.join(new_folder, pkl)) if os.path.lexists(
            os.path.join(new_folder, pkl)
        ) else None
        os.symlink(os.path.join(kitti_root, pkl), os.path.join(new_folder, pkl))
    # create symbolic links for testing subfolders
    for subfolder in testing_subfolders:
        os.remove(os.path.join(testing_folder, subfolder)) if os.path.lexists(
            os.path.join(testing_folder, subfolder)
        ) else None
        os.symlink(
            os.path.join(kitti_root, "testing", subfolder),
            os.path.join(testing_folder, subfolder),
        )
    # create symbolic links for training subfolders
    for subfolder in train_subfolders:
        os.remove(os.path.join(train_folder, subfolder)) if os.path.lexists(
            os.path.join(train_folder, subfolder)
        ) else None
        os.symlink(
            os.path.join(kitti_root, "training", subfolder),
            os.path.join(train_folder, subfolder),
        )

    # create a symbolic link for velodyne_eval to point to eval_image_path
    os.remove(os.path.join(train_folder, "velodyne_eval")) if os.path.lexists(
        os.path.join(train_folder, "velodyne_eval")
    ) else None
    os.symlink(eval_image_path, os.path.join(train_folder, "velodyne_eval"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--saved_path", default="results", help="your saved path for predicted results"
    )
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--num_workers", type=int, default=4)
    parser.add_argument("--nclasses", type=int, default=3)
    parser.add_argument("--no_cuda", action="store_true", help="whether to use cuda")
    # add additional argumuments to support evaluation the sparcpcgc reconstructed pcd
    parser.add_argument(
        "--pts_prefix",
        type=str,
        default="velodyne_reduced",
        help="the folder name under kitti root which stores the point cloud to be evaluated",
    )
    parser.add_argument(
        "--file_name_changer",
        type=str,
        default=r"\1.\2",
        help="the regex pattern to change the file name, e.g., from 000123.bin to 000123_reduced.ply, use r'\\1_reduced.ply'",
    )
    ...
    parser.add_argument(
        "--reference_pcd_prefix",
        type=str,
        default=None,
        help="the folder name under kitti root stores the reference point cloud file used in linear regression intensity filling or conform_to_target_intensity intensity filling",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="if not None, prepare the eval folder structure under the given path",
    )
    ...
    if args.prepare_eval is not None:
        # prepare the eval folder structure
        prepare_eval(args.prepare_eval)
        args.pts_prefix = "velodyne_eval"
